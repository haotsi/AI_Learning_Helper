...
```
AI_Learning_Helper/
├── ai-learning.html   # 整站唯一文件（约 334 KB）
├── README.md          # 本说明
└── CHANGELOG.md       # 更新记录
```
...
| 🎲 概率论与数理统计 | `#/prob`、`#/prob/p04` | **14 篇分章教程**：随机性与样本空间 → 条件概率与贝叶斯 → 随机变量与常见分布 → 期望/方差/协方差 → 大数定律与 CLT → 极大似然与假设检验 → 蒙特卡洛 → 交叉熵/KL 与生成模型；每篇「直觉 + 公式速查 + 可运行 Python 模拟」 |
| ✏️ 练习测验 | `#/quiz` | **60 题分级题库**：🟢 入门 18 / 🟠 进阶 26 / 🔴 挑战 16，覆盖 **AI / 数据结构 / 概率统计** 三大板块（数据结构 15 题、概率统计 12 题）；「全部」栏目按**题号顺序**排列，也可按难度筛选，完成后给出总分与各级得分 |
| 💻 编程实战 | `#/quiz` 页面下方 | **6 道 JavaScript 代码题**（两数之和 / 有效的括号 / 二分查找 / 最大子数组和 / 零钱兑换 / 无重复字符最长子串，共 42 个测试点）：代码在浏览器 **Worker 沙箱**本地判题，逐测试点比对对错、记录**耗时与内存占用**，`localStorage` 保存个人最佳成绩；每题附站内章节复习链接、示例与可展开的参考答案 |
...
「练习测验」页下方的编程实战模块提供类 LeetCode 的在线判题体验，**全程无后端**：

- **执行与判定**：用户提交的 JavaScript 代码被放入由 `Blob + Worker` 构建的沙箱中执行，逐测试点运行并与期望输出做深度比较（数值含 1e-9 容差）；覆盖编译错误、函数签名不符、运行时异常、死循环超时（4 秒上限，自动终止）等分支
- **耗时与内存**：逐测试点用 `performance.now()` 计时并汇总；内存通过 `performance.memory` 统计（该接口仅 Chromium 系浏览器提供，不可用时显示「—」，不影响判题）
- **成绩记录**：`localStorage`（key `learning-codelab-v1`）记录每题尝试次数、是否通过、个人最佳耗时/内存，题目列表实时显示 ✓
- **为什么是 JavaScript**：纯单文件、零依赖架构下，JS 是唯一能被浏览器原生安全执行的判题语言；C++ / Python 的在线编译需要自建后端判题机（如 Judge0）或约 10MB 的 WASM 工具链（Pyodide 等），不符合本站"双击即用"的定位。算法逻辑与 C++ / Python 完全通用
...
- **加编程题**：`window.APP_DATA.codeLab[]` 追加 `{id, title, level, tags, related, fn, starter, desc, tests, solution}`——`tests` 每项为 `{args: [...], expect: ...}`（必须 JSON 可序列化、确定性且答案唯一），`solution` 为参考实现；新增后务必先在 Node 里用参考解跑通全部测试点
